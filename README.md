--[[
    ASTA REMAKE – Full Hub (Simplificado) + ESP do Vasper
...
if isfile and isfile("AstaRemake.json") then
    ...
    if ok2 and type(d2)=="table" then
        ...
        if type(d2.espEnabled)=="boolean" then espEnabled=d2.espEnabled end
        if type(d2.showTracer)=="boolean" then showTracer=d2.showTracer end
    end
end
...
local function toggleTracer(on)
    showTracer = on
    BoxedESPOptions.tracer = showTracer
    refreshBoxedESP()
end
...
local function resetAllSettings()
    ...
    if showTracer then toggleTracer(false) end
    saveConfig()
end
...
-- ============================================================
-- FULL CONFIG LOAD (incluindo ESP)
-- ============================================================
pcall(function()
    ...
    if type(d.showTracer)=="boolean" then showTracer=d.showTracer end
end)

-- Aplica estado inicial (incluindo ESP)
pcall(function()
    ...
    -- Inicializa ESP se estiver ativo
    if espEnabled then
        task.spawn(function()
            task.wait(1)
            toggleESP(true)
...
;(function()
    ...
    Outer.Name="Outer"; Outer.Size=UDim2.new(0,340,0,520) -- aumentei altura para caber mais toggles
    Outer.Position=UDim2.new(0,6,0,54)
...
;(function()
    ...
    addSectLbl(vi,"ESP",0)
    local _, svEsp = addToggleRow(vi, "ESP", espEnabled, 1, nil, function(on)
        toggleESP(on)
        saveConfig()
    end)
    local _, svTracer = addToggleRow(vi, "Show Tracer", showTracer, 2, nil, function(on)
        toggleTracer(on)
        saveConfig()
    end)
...
if espEnabled then task.wait(0.5); toggleESP(true) end
if showTracer then task.wait(0.8); toggleTracer(true) end
CandyApplyCustomSky(currentSkyTheme)
buildMobileButtons()

end)() -- end GUI function

print("✅ ASTA REMAKE LOADED – Anti-Kick 100% robusto + Bat V2 (tecla B) + ESP integrado")
